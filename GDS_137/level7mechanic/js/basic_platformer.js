...
var parry;
	...
	player = new GameObject({x:150, color:"cyan"});
	enemy = new GameObject({x:900});
	gun = new GameObject();
	enemyGun = new GameObject();
	bullet = new GameObject();
	enemyBullet = new GameObject();
	explosion = new GameObject();
	parry = new GameObject();
	...
	enemyGun.height = 20;
	bullet.height = 15;
	enemyBullet.height = 15;
	gun.x = player.x + 50;
	enemyGun.x = enemy.x - 50;
	bullet.x = 1000000;
	enemyBullet.x = 1000000;
	gun.y = player.y - 10;
	enemyGun.y = enemy.y - 10;
	bullet.y = 1000000;
	enemyBullet.y = 1000000;
	gun.color = "black";
	enemyGun.color = "black";
...
function animate()
{
	...
	if(space && gun.angle == 0 && gun.x == player.x + 50 && bullet.bulletShoot == false)
	{
		...
		shootTimer = setTimeout(timedShot, 600);
	}
	...
	if(c && parry.canParry == true)
	{
		parry.x = player.x;
		parry.y = player.y;
		if (enemyBullet.hitTestObject(parry) && enemyBullet.x >= parry.x)
		{
			enemyBullet.vx = -enemyBullet.vx;
		}
		parry.canParry = false;
		clearTimeout(parryTimer2);
		parryTimer2 = setTimeout(parryLength, 100);
		clearTimeout(parryTimer);
		parryTimer = setTimeout(timedParry, 1000);
	}

	if(enemyBullet.bulletShoot == false)
	{
		enemyBullet.bulletShoot = true;
		enemyBullet.x = enemyGun.x;
		enemyBullet.y = enemyGun.y;
		enemyBullet.vx = -20;
		enemyBullet.vy = 0;
		clearTimeout(enemyShootTimer);
		enemyShootTimer = setTimeout(enemyTimedShot, 2000);
	}
...
function enemyTimedShot()
{
	enemyBullet.bulletShoot = false;
}
