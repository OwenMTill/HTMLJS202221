...
sky.img.src = `images/GameBackground.png`
...
for(let i=0; i<100; i++)
{
	bullets[i] = new GameObject({width:128, height:128, spriteData:projectileData})
	//bullets[i].img.src="images/mrt.jpg"
	bullets[i].makeSprite(projectileData)
	bullets[i].y=-10000
...
gameStates[`level1`] = function()
{
	...
	if(keys[` `] )
	{
		if(canShoot)
		{
			...
			bullets[currentBullet].vx = 25*wiz.dir;
			bullets[currentBullet].world = level;
			bullets[currentBullet].x = wiz.x-level.x + (wiz.dir) ;
